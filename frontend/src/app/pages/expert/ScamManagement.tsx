...
const ScamManagement = (props: { userId: string, }) => {
  // ** Expert side - Document Analysis with document viewer (Scam Managment)
  const params = useParams()
  const docId = params.docId // masterDocId
  const adminQId = params.adminQId
  ...
  const fetchingFile = async () => {
    try {
      if (docId) {
        const response = await axios.get(`${backendUrl}/scam-alert/get_selected_files/${docId}`) //docId = masterDocId
  ...
  const handleReview = async () => {
    try {
      if (!adminNotes.trim()) {
        return toast.warning("Please provide a reason for your decision");
      }
      setLoading(true)
      const res = await axios.post(`${backendUrl}/scam-alert/admin/decide`, {
        "document_id": docId,
        "decision": decision,
        "notes": adminNotes,
        "reviewer": props.userId
      })
      ...
      toast.error("Failed to submit decision")
      console.log("Error submit decision : ", error)
    } finally {
      setLoading(false)
    }
  }
                      ...
                      {selectedDocument.expertReview === true ? (
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-md overflow-hidden ">
                          ...
                          <div className="bg-slate-50 border-b border-slate-200 p-4 flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-tight">Final Verification</h3>
                              <p className="text-[11px] text-slate-500 font-medium">TRUSTLENS AI PIPELINE</p>
                            </div>
                          </div>
                          ...
                          <div className="p-6">
                            ...
                            <div className="mb-6 p-3 bg-red-50 border-l-4 border-red-500 rounded-r-lg">
                              <p className="text-xs text-red-800 leading-relaxed">
                                <span className="font-bold">Flagged:</span> High-confidence scam detected. Verify indicators before publishing.
                              </p>
                            </div>

                            {/* Decision Selection Group */}
                            <div className="mb-6">
                              <label className="block text-xs font-semibold text-slate-500 uppercase mb-3">Assessment</label>
                              <div className="grid grid-cols-2 gap-3">
                                <button
                                  onClick={() => setDecision("APPROVED")}
                                  className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border-2 transition-all font-semibold text-sm ${decision === "APPROVED"
                                ...
                                <button
                                  ...
                                  className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border-2 transition-all font-semibold text-sm ${decision === "REJECTED"
                            ...
                            {/* Notes Section */}
                            <div className="mb-6">
                              <label className="block text-xs font-semibold text-slate-500 uppercase mb-2">Reviewer Notes</label>
                              <textarea
                                rows={3}
                                className="w-full px-4 py-3 text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder:text-slate-400 resize-none"
                            ...
                            <button
                              ...
                              className={`w-full py-3.5 rounded-xl font-bold text-sm tracking-wide transition-all shadow-lg flex items-center justify-center gap-2 ${!decision
                            ...
                            >
                              {loading ? (
                                ...
                                `Confirm ${decision === "APPROVED" ? "Publication" : "Rejection"}`
                              )}
                            </button>
                          </div>
                        </div>
